...
function startGame() {
    ...
    document.getElementById("score").innerHTML = 'Текущий счет: ' + score.toString();
}
    ...
    if (unit_classes.includes('food-unit')) {
        ...
        if(score % 5 == 0){
            ...
            while(obstacles.length != 0){
                var obstacle_cell_classes = obstacles[0].getAttribute('class').split(' ');                
                obstacles[0].setAttribute('class', obstacle_cell_classes[0] + ' ' + obstacle_cell_classes[1])
            }      
            createObstacle();      
        }
        
    }
    else if(unit_classes.includes('obstacle-unit')){
        finishTheGame();
    }
    return check;
}
...
function createObstacle() {
    for(var difficulty = 0; difficulty < 5; difficulty ++){
        ...
        while (!obstacleCreated) { //пока еду не создали
            ...
            var obstacle_cell = document.getElementsByClassName('cell-' + obstacle_x + '-' + obstacle_y)[0];
